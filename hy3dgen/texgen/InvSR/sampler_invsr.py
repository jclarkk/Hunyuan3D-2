...
def pil_to_tensor(image: Image.Image, bgr2rgb=False, out_type=torch.float32):
    """
    Converts a PIL Image to a 4D torch tensor (1, C, H, W).

    Args:
        image (PIL.Image): Input PIL Image
        bgr2rgb (bool): Whether to convert BGR to RGB
        out_type (torch.dtype): Desired torch dtype

    Returns:
        torch.Tensor: (1, C, H, W) tensor
    """
    ...
    if image_np.ndim == 2:  # Grayscale
        image_np = image_np[:, :, None]  # Convert to (H, W, 1)

    # Pass through img2tensor function
    image_tensor = util_image.img2tensor(image_np, bgr2rgb=bgr2rgb, out_type=out_type)
...
def numpy_to_pil(image_np):
    """
    Convert numpy array output from sample_func to a PIL Image.
    """
    # Ensure the image is in the range [0,255] and dtype uint8
    image_np = (image_np * 255).clip(0, 255).astype(np.uint8)
    return Image.fromarray(image_np)


def process_image(sampler, input_image):
    """
    Call sample_func and convert the output to a PIL Image.

    Args:
        sampler: Instance of InvSamplerSR
        input_image: Torch tensor (1, C, H, W) in [0,1] range

    Returns:
        PIL Image
    """
    output_np = sampler.sample_func(input_image.cuda()).squeeze(0)  # H x W x C, [0,1]
