...
def get_block_representation(
    ...
    block_coords = np.concatenate([patch_id[..., None], block_id[..., None], offset_id[..., None]], axis=-1).astype(
        np.int64)
...
def decode_block(sequence, compressed=True, patch_size=4, block_size=8, offset_size=16):
    ...
    if compressed:
        ...
        for token_id in range(len(sequence)):
            if block_size ** 3 + offset_size ** 3 + patch_size ** 3 > sequence[
                token_id] >= block_size ** 3 + patch_size ** 3:  # offset
...
def deserialize(sequence, patch_size=4, block_size=8, offset_size=16, compressed=True, special_token=-2,
                use_special_block=True):
    ...
    for i in range(len(sequence)):
        ...
        if not use_special_block and (sequence[i] == special_token or i == len(sequence) - 1):
            ...
            sub_seq = decode_block(sub_seq, compressed=compressed, patch_size=patch_size, block_size=block_size,
                                   offset_size=offset_size)
            start_idx = i + 1

        elif use_special_block and \
                (special_block_base <= sequence[i] < special_block_base + patch_size ** 3 or i == len(
                    sequence) - 1):  # center
            if i != 0:
